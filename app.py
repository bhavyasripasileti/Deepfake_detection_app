...
from inference import get_model, predict_image, predict_video
...
def sidebar():
    with st.sidebar:
        ...
        st.caption("EfficientNetB4 + MTCNN + Grad-CAM")
        st.divider()
...
def render_gallery(result):
    ...
    def tag(s):
        ...
        return f"<div class='frame-tag' style='background:{c}22;border:1px solid {c};color:{c}'>{l} {s:.0%}</div>"
...
def run_video(uploaded, model, settings):
    ...
    try:
        ...
        st.video(tmp_path)
        with st.spinner(f"Extracting {settings['num_frames']} frames and analysing …"):
            t0     = time.time()
            result = predict_video(tmp_path, model, settings["num_frames"])
            elapsed = time.time() - t0
        st.subheader("Result")
        if "error" not in result:
            st.success(f"✅ {result['n_faces']} face crops  |  {elapsed:.1f} s")
...
def main():
    ...
    st.markdown("<h1 style='margin-bottom:0'>🕵️ Deepfake Face Detection</h1>",
                unsafe_allow_html=True)
    st.markdown("<p style='color:#888;margin-top:0'>Upload an image or video to detect AI-manipulated faces.</p>",
                unsafe_allow_html=True)
    st.divider()
...
2. **EfficientNetB4** extracts features from the face crop.
3. A **Dense head** outputs the fake probability (0–1).
4. **Grad-CAM** highlights the regions that drove the decision.
5. For video, scores are **averaged** across all sampled frames.
            """)
        return
